...
class MovieFinderTest {
	...
	MovieStorage movieStorageMock = Mockito.mock(MovieStorage.class); // mock injection
	MovieStorage storageStub = new MovieStorageStub(); // constructor injection
	
    MovieFinder movieFinder = new MovieFinder(movieStorageMock);

    @BeforeEach
    void setUp() {
        Mockito.when(movieStorageMock.getAll()).thenReturn(MovieFixtures.MOVIES);
    }
